...
class WebSearchNode(BaseNode):
    ...
    def __call__(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            if not query or len(query.strip()) < 2:
                ...
                return {"documents": [], "data_source": "none"}

            if self.search_tool is None:
                logger.warning(f"[{self.node_name}] No search tool configured (TAVILY_API_KEY missing)")
                return {"documents": [], "data_source": "none"}
            
            enhanced_query = self._enhance_query_for_legal_search(query)
            logger.info(f"[{self.node_name}] Searching web for: '{enhanced_query}'")
            
            # Lấy documents đã có từ state (ví dụ từ Neo4j)
            existing_documents = state.get("documents", [])
            
            # Thực hiện search web
            new_web_documents = self._search_web(enhanced_query)
            
            logger.info(
                f"[{self.node_name}] Found {len(new_web_documents)} web documents. "
                f"Appending to {len(existing_documents)} existing documents."
            )
            
            # Gộp kết quả: Giữ lại ưu tiên DB trước, Web sau
            combined_documents = existing_documents + new_web_documents
            
            result = {
                "documents": combined_documents, 
                "data_source": "internet" # Vẫn đánh dấu là có dùng internet
            }
            ...
            return {"documents": [], "data_source": "web_error"}
    ...
    def _search_web(self, query: str, max_results: int = 3) -> List[LegalDocument]:
        """
        ...
        Returns:
            List[LegalDocument] đã được convert bởi TavilySearchService
        """
        try:
            ...
            logger.debug(f"[{self.node_name}] Search returned {len(results)} documents")
            return results
