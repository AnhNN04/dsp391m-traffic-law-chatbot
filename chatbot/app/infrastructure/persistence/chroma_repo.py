...
class ChromaRepo(IVectorStore):
    ...
    def __init__(
        ...
        try:
            ...
            logger.info("ChromaDB repository initialized successfully", collection=collection_name)
            
        except Exception as e:
            logger.error("Failed to initialize ChromaDB repository", error=str(e), persist_directory=persist_directory)
    ...
    def search(
        ...
        try:
            ...
            logger.error("Vector search failed", error=str(e), query=query[:100], k=k)
    ...
    def search_similarity(self, query: str, k: int = 5) -> List[LegalDocument]:
        """Alias for search() — used by RetrievalNode."""
        return self.search(query, k=k)

    def similarity_search_with_score(self, query: str, k: int = 5) -> List:
        """Return raw (Document, score) tuples from Chroma."""
        try:
            return self.client.similarity_search_with_score(query=query, k=k)
        except Exception as e:
            logger.error("similarity_search_with_score failed", error=str(e))
            return []

    def max_marginal_relevance_search(self, query: str, k: int = 5, fetch_k: int = 20) -> List[LegalDocument]:
        """MMR search for diversity — fallback to regular search."""
        return self.search(query, k=k)

    def health_check(self) -> bool:
        """Check if ChromaDB is accessible."""
        try:
            self.client._collection.count()
            return True
        except Exception:
            return False
