...
class Container:
    ...
    def openai_service(self):
        """OpenAI Service - disabled, using Groq instead."""
        logger.warning("OpenAI service is not configured. Using Groq for all nodes.")
        return None

    @property
    def groq_service(self) -> "GroqService":
        """Khởi tạo Groq Service (Fast LLM) - Dùng cho tất cả các node."""
        if self._groq_service is None:
            from app.infrastructure.external_services.groq_llm_service import GroqLLMService
            self._groq_service = GroqLLMService(
    ...
    def tavily_service(self) -> Optional["TavilyService"]:
        ...
        if self._tavily_service is None and self.settings.has_web_search():
            ...
            from app.infrastructure.external_services.tavily_search_service import TavilySearchService
            self._tavily_service = TavilySearchService(api_key=self.settings.TAVILY_API_KEY)
    ...
    def rewrite_node(self) -> "RewriteNode":
        if self._rewrite_node is None:
            ...
            self._rewrite_node = RewriteNode(smart_llm=self.groq_service)
    ...
    def router_node(self) -> "RouterNode":
        if self._router_node is None:
            ...
            self._router_node = RouterNode(smart_llm=self.groq_service)
    ...
    def grade_node(self) -> "GradeNode":
        if self._grade_node is None:
            ...
            self._grade_node = GradeNode(smart_llm=self.groq_service)
    ...
    def ask_human_node(self) -> "AskHumanNode":
        if self._ask_human_node is None:
            ...
            self._ask_human_node = AskHumanNode(smart_llm=self.groq_service)
    ...
    def web_search_node(self) -> "WebSearchNode":
        if self._web_search_node is None:
            ...
            self._web_search_node = WebSearchNode(
                search_tool=self.tavily_service  # None if TAVILY_API_KEY not set
            )
    ...
    def generate_node(self) -> "GenerateNode":
        if self._generate_node is None:
            ...
            self._generate_node = GenerateNode(smart_llm=self.groq_service)
    ...
    def mongo_history(self):
        ...
        if not hasattr(self, "_mongo_history"):
            ...
            self._mongo_history = MongoHistory()  # reads settings internally
